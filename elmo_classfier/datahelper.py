...
class UnicodeCharsVocabularyV1(Vocabulary):
    """Vocabulary containing character-level and word level information.
    ...
    from the pre-trained model will be useless.
    """
    def __init__(self, filename, char_vocab_filename,max_word_length, **kwargs):
        super(UnicodeCharsVocabularyV1, self).__init__(filename, **kwargs)
        self._max_word_length = max_word_length
        self.char_vocab=Vocabulary(char_vocab_filename,True)
        # char ids 0-255 come from utf-8 encoding bytes
        # assign 256-300 to special chars
        max_pos=len(self.char_vocab._word_to_id)
    ...
    def _convert_word_to_char_ids(self, word):
        ...
        #word_encoded = word.encode('utf-8', 'ignore')[:(self.max_word_length-2)]
        word_encoded=word[:(self.max_word_length-1)]
        code[0] = self.bow_char
        for k, chr_id in enumerate(word_encoded, start=1):
            code[k] = self.char_vocab._word_to_id[
                chr_id] if chr_id in self.char_vocab._word_to_id else self.char_vocab.unk
        code[len(word_encoded) + 1] = self.eow_char
...
class Batcher(object):
    ...
    '''
    def __init__(self, lm_vocab_file: str, lm_char_vocab_file:str,max_token_length: int):
        '''
        lm_vocab_file = the language model vocabulary file (one line per
            token)
        max_token_length = the maximum number of characters in each token
        '''
        self._lm_vocab = UnicodeCharsVocabularyV1(
            lm_vocab_file,lm_char_vocab_file, max_token_length
        )
...
def read_data(file_path,with_label=True):
    x=[]
    y=[]
...
def batch(x,y,batch_size=128,epochs=5):
    x_len=len(x)
    for _ in range(epochs):
        start_index=0
        for start_index in range(0,x_len,batch_size):
            end_index=min(start_index+batch_size,x_len)
