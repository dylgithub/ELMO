...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            #####添加
            #用word2vec或glove训练的词向量存储地址，
            # 需注意的是根据下面代码，词向量的维度要和RNN内部隐藏节点的个数相等
            embedding_addr='/data/word_embedding.txt'
            import gensim
            w2v_model=gensim.models.KeyedVectors.load_word2vec_format(embedding_addr)
            #下面的n_tokens_vocab是词表的大小，包含<S></S><UNK>
